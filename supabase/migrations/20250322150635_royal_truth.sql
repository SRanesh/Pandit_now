/*
  ...
  1. Changes
    ...
    - Add role and profile data handling
    - Fix trigger execution timing
...
RETURNS TRIGGER AS $$
DECLARE
  default_role text := COALESCE(NEW.raw_user_meta_data->>'role', 'devotee');
BEGIN
...
RETURNS TRIGGER AS $$
BEGIN
  -- Update profile when metadata changes
